...
export const TABELLE_SCHEMA = [
  {
    ...
    fields: [
      testo("Nome"),
      testo("Cognome"),
      testo("NomeSistema", "Come si chiama la dashboard, scelto da Config. Vuoto = il nome di partenza."),
      testo("Ruolo"),
      testo("Citta"),
      ...
      testoLungo("Settori", "JSON: l'elenco ordinato delle colonne del Kanban dei Cicli Aperti. Vuoto = i quattro predefiniti."),
      testo("PasswordHash", "La password scelta da Config, come PBKDF2 (sale$hash), mai in chiaro. Vuoto = vale DASHBOARD_PASSWORD."),
      testoLungo("Interfaccia", "JSON: moduli attivi, ordine di menu e Home, tema. Vuoto = tutto attivo, ordine standard."),
      dataOra("ConfiguratoIl", "Quando è stato completato il wizard del primo accesso. Vuoto = al prossimo accesso riparte."),
    ],
  },
  {
    name: "Persone",
    description: "Le persone della Rubrica e del CRM. Ogni task può essere legato a una o più di queste.",
    fields: [
      testo("Nome"),
      testo("Cognome"),
...
export async function creaTabelle(base, chiave) {
  ...
  for (const tabella of TABELLE_SCHEMA) {
    ...
    const esistente = perNome.get(tabella.name);
    if (esistente) {
      // La tabella c'è già: si confrontano i campi, e quelli nati dopo si
      // aggiungono. È quello che rende indolore aggiornare una dashboard
      // vecchia: si rivisita /api/admin/tabelle e i campi nuovi compaiono.
      // I campi esistenti non si toccano mai, né i tipi né le scelte.
      const nomi = new Set((esistente.fields ?? []).map((f) => f.name));
      const mancanti = fields.filter((f) => !nomi.has(f.name));
      for (const campo of mancanti) {
        await chiama("POST", `${radice}/${esistente.id}/fields`, campo);
      }
      if (mancanti.length) campiAggiunti[tabella.name] = mancanti.map((f) => f.name);
      else saltate.push(tabella.name);
      continue;
    }
  ...
  return { create, saltate, campiAggiunti };
}
