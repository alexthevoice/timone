...
export async function leggiPersone() {
  ...
  return righe.map((r) => ({
    ...
    nome: r.Nome ?? "",
    cognome: r.Cognome ?? "",
    // Il nome da mostrare: chi è stato creato al volo ha tutto nel Nome, chi
    // passa dalla Rubrica ha il cognome nel suo campo. Qui si ricompone.
    nomeCompleto: [r.Nome, r.Cognome].map((x) => String(x ?? "").trim()).filter(Boolean).join(" "),
...
/* I campi della Rubrica, dal nome usato dal browser al nome della colonna.
   Un elenco chiuso: quello che non sta qui non si scrive, chiunque lo mandi. */
const CAMPI_PERSONA = {
  nome: "Nome",
  cognome: "Cognome",
...
function campiPersona(patch) {
  const campi = {};
  for (const [chiave, colonna] of Object.entries(CAMPI_PERSONA)) {
    if (patch[chiave] === undefined) continue;
    const valore = String(patch[chiave] ?? "").trim();
    // Tipo è un elenco a scelte: vuoto va scritto come null, non come "".
    campi[colonna] = chiave === "tipo" ? valore || null : valore;
  }
  return campi;
}

export async function creaPersona(dati) {
  const campi = campiPersona(dati);
  if (!campi.Nome && !campi.Cognome) throw new Error("Serve almeno il nome o il cognome.");
  return crea(TABELLE.persone, { ...campi, CreatoIl: adessoIso() });
}

export async function aggiornaPersona(id, patch) {
  return aggiorna(TABELLE.persone, id, campiPersona(patch));
}

/**
 * Eliminare una persona non tocca i suoi task: Airtable scioglie il
 * collegamento da solo, e le righe restano con la loro storia.
 */
export async function eliminaPersona(id) {
  return elimina(TABELLE.persone, id);
}
...
export async function leggiTask({ includiCompletati = false } = {}) {
  ...
  return righe
    ...
    .map((r) => {
      ...
      const nomi = personeId.map((x) => perId.get(x)?.nomeCompleto).filter(Boolean);
      const base = {
        id: r.id,
        titolo: r.Titolo ?? "",
        nota: r.Nota ?? "",
