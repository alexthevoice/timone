...
async function chiama(metodo, indirizzo, corpo) {
  ...
  if (!r.ok) {
    throw new Error(`Airtable ${r.status}: ${JSON.stringify(dati).slice(0, 300)}`);
  }
  return dati;
}
...
console.log(`Base ${BASE}: ${esistenti.length} tabelle esistenti.`);

for (const tabella of TABELLE) {
  if (perNome.has(tabella.name)) {
    console.log(`· ${tabella.name}: c'è già, la salto.`);
    continue;
  }
  ...
  console.log(`✓ ${tabella.name}: creata con ${fields.length} campi.`);
}
